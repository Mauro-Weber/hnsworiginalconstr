...
//                    if (candidate_id == 0) continue;
    ...
    template <bool has_deletions, bool collect_metrics = false>
    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
    searchBaseLayerST_infl(tableint ep_id, const void *data_point, size_t ef, BaseFilterFunctor* isIdAllowed = nullptr) const {
        VisitedList *vl = visited_list_pool_->getFreeVisitedList();
        vl_type *visited_array = vl->mass;
        vl_type visited_array_tag = vl->curV;
...
//                    if (candidate_id == 0) continue;
#ifdef USE_SSE
                _mm_prefetch((char *) (visited_array + candidatePair.second), _MM_HINT_T0);
                _mm_prefetch(data_level0_memory_ + (candidatePair.second) * size_data_per_element_ + offsetData_,
                             _MM_HINT_T0);  ////////////
#endif
                if (!(visited_array[candidatePair.second] == visited_array_tag)) {
                    ...
                    //char *currObj1 = (getDataByInternalId(candidate_id));
                    //dist_t dist = fstdistfunc_(data_point, currObj1, dist_func_param_);
                    std::cout << "Candidato: " << candidatePair.second << " dist: " << candidatePair.first << std::endl;
    ...
    */
    ...
    searchKnn(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        ...
        std::cout << "ENTER POINT CHOSE: " << currObj << std::endl;
        std::cout << "Find the closest neighbor" << std::endl;
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates_k1;
        if (num_deleted_) {
            top_candidates_k1 = searchBaseLayerST<true, true>(
                currObj, query_data, std::max(ef_, k), isIdAllowed);
        } else {
            top_candidates_k1 = searchBaseLayerST<false, true>(
                currObj, query_data, std::max(ef_, k), isIdAllowed);
        }

        while (top_candidates_k1.size() > 1) {
            top_candidates_k1.pop();
        }
        std::cout << "K_1: " << top_candidates_k1.top().second << std::endl;
        std::pair<dist_t, tableint> k1_rez = top_candidates_k1.top();

        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;

        if (num_deleted_) {
            top_candidates = searchBaseLayerST_infl<true, true>(
                k1_rez.second, query_data, k, isIdAllowed);
        } else {
            top_candidates = searchBaseLayerST_infl<false, true>(
                k1_rez.second, query_data, k, isIdAllowed);
        }
